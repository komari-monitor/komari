...
func (m *PingTaskManager) Reload(pingTasks []models.PingTask) error {
	...
	allTasks := append([]models.PingTask(nil), pingTasks...)
	scheduler.Remove("route:periodic")
	return scheduler.AddContextFunc("route:periodic", scheduler.Every(time.Minute), true, func(ctx context.Context) {
		...
		for _, task := range allTasks {
			...
			for _, clientUUID := range targetPingClientUUIDs(task) {
				...
				dispatchRouteTrace(clientUUID, task, target, interval, false)
			}
		}
	})
}
