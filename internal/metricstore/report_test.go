...
func TestWriteReportCountsTBScaleTrafficDeltas(t *testing.T) {
	...
	report := v1.Report{
		...
		UpdatedAt: base,
		Uptime:    10_000,
		Network:   v1.NetworkReport{TotalUp: 2 * oneTB, TotalDown: 3 * oneTB},
	}
	if _, err := WriteReport(ctx, report); err != nil {
		t.Fatalf("write first report: %v", err)
	}
...
func TestWriteReportRestoresTBScaleCountersFromStore(t *testing.T) {
	...
	report := v1.Report{
		...
		UpdatedAt: base,
		Uptime:    10_000,
		Network:   v1.NetworkReport{TotalUp: 2*oneTB + 123, TotalDown: 4*oneTB + 456},
	}
	if _, err := WriteReport(ctx, report); err != nil {
		t.Fatalf("write first report: %v", err)
	}
...
func TestWriteReportCountsTrafficAfterHighRateCounterWrap(t *testing.T) {
	...
	report := v1.Report{
		...
		UpdatedAt: base,
		Uptime:    10_000,
		Network:   v1.NetworkReport{TotalUp: 3*oneGB + 900_000_000, TotalDown: 3*oneGB + 800_000_000},
	}
	if _, err := WriteReport(ctx, report); err != nil {
		t.Fatalf("write first report: %v", err)
	}
